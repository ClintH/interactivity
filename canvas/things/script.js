...
// valueBounce is a helper function that returns a function (!)
const scaleBounce = valueBounce(0.5, 5, 0.001); // Very slowly zoom in and our
...
function drawThing(thing, ctx) {
  ...
  const distance = thing.distance * ctx.canvas.width / 3;

  // Compute location of thing according to current angle around its own origin point
  const radians = degreesToRadians(thing.angle % 360);
  const pointCoord = polarToCartesian(radians, distance, thing.originX, thing.originY);
...
// Keeps track of a value and 'bounces' it between
// low and high bounds by a set increment
